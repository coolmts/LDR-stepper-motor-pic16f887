...
// last_angle_x will  save the last  angle of motors
 //last_direction_x will  save the last rotation direction of motors
...
// a is the array  to move motor one step clockwise for each loop (a[0to7])
const char a [] = {0xEE,0xCC,0xDD,0x99,0xBB,0x33,0x77,0x66};
// ac is the array  to move motor one step counter-clockwise for each loop (a[0to7])
const char ac [] = {0x66,0x77,0x33,0xBB,0x99,0xDD,0xCC,0xEE};
const char array_size = 8 ; // the size of the a[] 
//////////////////////////////////////////////////////////////////
char ADCchannel []={1,2,3,4,5,6,7,8,9}; // select which analog channel to  read from
char numofchannel = 8; // number of ADC in the array ADCchannel
char channel ; 
int result ; 
/*
 * 
...
void __interrupt() ISA(){
    if(ADIF){
         ...
         // if result < 128 mean the LDR does not see light
         // you can  change this value to serve your job
        if (result < 128){
            which_pin(channel);
            //__delay_s(1);
           PORTD = channel ; // just for testing
          __delay_ms(100);
            ADIF = 0 ;  // turn off the interrupt
         }
        else{
           //  which_pin(5);
            ADIF = 0 ;}
